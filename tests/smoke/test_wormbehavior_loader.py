"""Real-format smoke test for WormBehaviorDBLoader.

Default fixture: ``tests/fixtures/wormbehavior_db/`` (committed minimal HDF5
per anchor record). Override with real Zenodo extract by setting
``JEPA_WORM_WORMBEHAVIOR_FIXTURE_DIR`` to a directory laid out as
``<root>/<record_id>/*.hdf5``.
"""
...
_ENV_VAR = "JEPA_WORM_WORMBEHAVIOR_FIXTURE_DIR"
_COMMITTED_FIXTURE = Path(__file__).resolve().parents[1] / "fixtures" / "wormbehavior_db"
_MIN_SAMPLES = 2
...
def test_wormbehavior_loader_on_real_subset() -> None:
    fixture = _resolve_fixture_dir()
    loader = WormBehaviorDBLoader(fixture, clip_frames=8, image_size=(64, 64))
